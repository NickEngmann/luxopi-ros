...
class HID:
    ...
    def find(vid=0x2886, pid=0x0018):
        # First, let's try to find any device with this vendor ID
        devices = list(usb.core.find(find_all=True, idVendor=vid))
        if devices:
            print(f"Found {len(devices)} device(s) with vendor ID {hex(vid)}:")
            for dev in devices:
                print(f"  PID: {hex(dev.idProduct)}")
        
        dev = usb.core.find(idVendor=vid, idProduct=pid)
        if not dev:
            print(f"No device found with VID: {hex(vid)}, PID: {hex(pid)}")
            # Try with a different common PID
            dev = usb.core.find(idVendor=vid, idProduct=0x001a)
            if dev:
                print(f"Found device with PID: 0x001a instead")
            else:
                return
