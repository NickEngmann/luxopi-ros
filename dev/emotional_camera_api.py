...
if args.fps_limit is None:
    args.fps_limit = 5  # Increased from 8 to 15 for better responsiveness
    print(
        f"\nFPS limit set to {args.fps_limit} for RVC2. Use --fps_limit flag to customize.\n"
    )
...
with dai.Pipeline(device) as pipeline:
    ...
    det_model_description = dai.NNModelDescription.fromYamlFile(
        "./depthai_models/yunet.RVC2.yaml"
    )
    ...
    rec_model_description = dai.NNModelDescription.fromYamlFile(
        "./depthai_models/emotion_recognition.RVC2.yaml"
    )
