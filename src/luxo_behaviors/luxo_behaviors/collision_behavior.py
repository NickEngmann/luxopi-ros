...
class CollisionBehavior:
    ...
    def setup_collision_behavior(self):
        ...
        if not self.node.has_parameter('soft_limit_distance'):
            self.node.declare_parameter('soft_limit_distance', 15.0)  # Match warning threshold (increased from 9.0)
        if not self.node.has_parameter('hard_limit_distance'):
            self.node.declare_parameter('hard_limit_distance', 6.0)  # Match danger threshold (increased from 4.0)
