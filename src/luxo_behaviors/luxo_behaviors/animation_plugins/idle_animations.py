...
class GentleSwayAnimation(AnimationPlugin):
    ...
    def get_keyframes(self) -> Tuple[List[List[float]], List[float]]:
        ...
        keyframes = [
            ...
            [base_pos, -0.88, 1.32, 1.42, -1.6, 10.5],                        # Subtle inhale
            [base_pos + sway_amount*0.15, -0.78, 1.25, 1.38, -1.2, 11.0],     # Begin shift
            [base_pos + sway_amount*0.4, -0.7, 1.18, 1.45, -1.8, 10.0],       # Lean into movement
            [base_pos + sway_amount*0.45, -0.68 + breath_lift, 1.16, 1.47, -0.9, 13.5], # Micro shoulder adjust
            [base_pos + sway_amount*0.85, -0.6, 1.08, 1.52, -2.1, 11.5],      # Near full sway
            [base_pos + sway_amount, -0.58 - breath_lift, 1.05, 1.54, -0.8, 10.0], # Full sway with breath
            [base_pos + sway_amount*0.7, -0.65, 1.12, 1.45, -1.9, 11.0],      # Begin exhale return
            [base_pos + sway_amount*0.2, -0.72, 1.2, 1.38, -1.1, 12.0],       # Through center
            [base_pos - sway_amount*0.15, -0.76, 1.22, 1.36, -2.0, 10.5],     # Begin left shift
            [base_pos - sway_amount*0.5, -0.73, 1.18, 1.4, -0.85, 14.0],      # Head tilt moment
            [base_pos - sway_amount*0.9, -0.62, 1.1, 1.48, -1.7, 11.0],       # Near full left
            [base_pos - sway_amount, -0.6 + breath_lift*0.5, 1.08, 1.46, -0.9, 10.0], # Full left with settle
            [base_pos - sway_amount*0.4, -0.74, 1.2, 1.4, -1.6, 11.5],        # Gentle return
            [base_pos - 0.05, -0.8, 1.26, 1.41, -1.3, 12.5],                  # Balance check
            [base_pos, -0.8, 1.29, 1.4, -1.5, 11.0],                         # Near home
...
class CuriousExplorationAnimation(AnimationPlugin):
    ...
    def get_keyframes(self) -> Tuple[List[List[float]], List[float]]:
        ...
        keyframes = [
            ...
            [base_pos - 0.1, -0.65, 1.15, 1.3, -1.2, 16.0],             # Something catches attention
            [base_pos - 0.55, -0.55, 1.05, 1.2, -1.9, 19.0],            # Quick glance left
            [base_pos - 0.5, -0.6, 1.1, 1.25, -0.85, 15.0],              # Pause - wait what?
            [base_pos - 0.45, -0.45, 0.9, 1.05, -2.0, 17.5],            # Double-take lean forward
            [base_pos - 0.42, -0.4, 0.85, 1.0, -0.8, 14.5],             # Squint for detail
            [base_pos - 0.48, -0.42, 0.87, 1.08, -1.8, 16.5],           # Head tilt confused
            [base_pos - 0.4, -0.25, 0.65, 0.85, -0.9, 12.0],            # Lean way in to inspect
            [base_pos - 0.35, -0.3, 0.7, 0.95, -2.1, 20.0],             # Aha! I see it
            [base_pos - 0.2, -0.65, 1.15, 1.4, -0.75, 15.5],             # Pull back to process
            [base_pos, -0.95, 0.85, 1.65, -1.7, 11.5],                  # Look up thinking about it
            [base_pos + 0.65, -0.5, 1.05, 1.35, -0.85, 18.0],            # Glance right to compare
            [base_pos + 0.2, -0.6, 1.12, 1.3, -1.9, 16.0],              # Nod - I understand now
            [base_pos, -0.75, 1.23, 1.37, -1.1, 13.5],                  # Satisfied exhale
            [base_pos, -0.82, 1.28, 1.39, -1.6, 12.0],                  # Return to rest
            [base_pos, -0.65, 1.2, 1.0, -1.5, 13.0]                     # Home 2
        ]
...
class BreathingAnimation(AnimationPlugin):
    ...
    def get_keyframes(self) -> Tuple[List[List[float]], List[float]]:
        ...
        keyframes = [
            ...
            [base_pos, -0.88, 1.32, 1.43, -1.3, 10.0],                  # Mental preparation
            [base_pos + sway, -0.95, 1.34, 1.5, -1.8, 10.0],            # Begin deep inhale
            [base_pos, -1.05, 1.38, 1.6, -0.9, 10.5],                   # Chest expanding
            [base_pos - sway, -1.1, 1.42, 1.68, -2.0, 10.0],            # Full expansion (at limit)
            [base_pos, -1.08, 1.41, 1.66, -0.8, 10.0],                  # Hold at peak
            [base_pos + sway*0.5, -1.06, 1.4, 1.64, -1.7, 11.0],        # Slight waver in hold
            [base_pos, -1.0, 1.36, 1.55, -1.1, 10.5],                   # Begin controlled exhale
            [base_pos - sway, -0.85, 1.3, 1.45, -1.9, 10.0],            # Releasing downward
            [base_pos, -0.65, 1.2, 1.25, -0.85, 10.5],                   # Deep exhale compression
            [base_pos, -0.62, 1.18, 1.22, -2.2, 10.0],                  # Bottom of breath pause
            [base_pos, -0.72, 1.24, 1.32, -0.75, 11.5],                  # Small recovery inhale
            [base_pos, -0.8, 1.28, 1.38, -1.6, 11.0],                   # Settling
            [base_pos, -0.84, 1.2, 1.39, -1.3, 10.5],                   # Peaceful state
            [base_pos, -0.65, 1.2, 1.0, -1.5, 10.0]                     # Home 2
        ]
...
class AttentiveListeningAnimation(AnimationPlugin):
    ...
    def get_keyframes(self) -> Tuple[List[List[float]], List[float]]:
        ...
        keyframes = [
            [base_pos, -0.65, 1.2, 1.0, -1.5, 12.5],                    # Home 2
            [base_pos, -0.7, 1.2, 1.35, -1.3, 14.0],                    # Shift to attention
            [base_pos, -0.65, 1.15, 1.3, -1.8, 15.5],                   # Eyebrow raise interest
            [base_pos - 0.15, -0.5, 1.0, 1.15, -0.9, 13.0],             # Lean in curious
            [base_pos - 0.12, -0.52, 1.02, 1.12, -2.0, 16.0],           # Micro nod understanding
            [base_pos - 0.35, -0.55, 1.05, 1.2, -0.8, 14.5],            # Head tilt questioning
            [base_pos - 0.32, -0.62, 1.12, 1.28, -1.9, 13.5],           # Processing information
            [base_pos - 0.3, -0.58, 1.08, 1.24, -1.0, 15.0],            # Another micro nod
            [base_pos + 0.1, -0.6, 1.1, 1.3, -1.7, 14.0],               # Shift weight/position
            [base_pos + 0.38, -0.58, 1.08, 1.32, -0.85, 14.5],           # Head tilt other way
            [base_pos + 0.15, -0.48, 0.98, 1.18, -2.1, 17.0],           # Deep understanding nod
            [base_pos, -0.72, 1.22, 1.42, -0.75, 12.0],                  # Pull back to think
            [base_pos, -0.68, 1.18, 1.38, -1.8, 13.5],                  # Understanding dawns
            [base_pos, -0.55, 1.05, 1.25, -1.2, 15.0],                  # Agreement gesture forward
            [base_pos, -0.78, 1.26, 1.38, -1.6, 11.5],                  # Settle back satisfied
            [base_pos, -0.65, 1.2, 1.0, -1.5, 12.5]                     # Home 2
        ]
...
class ScanningWatchAnimation(AnimationPlugin):
    ...
    def get_keyframes(self) -> Tuple[List[List[float]], List[float]]:
        ...
        keyframes = [
            [0.0, -0.65, 1.2, 1.0, -1.5, 13.0],                          # Home 2
            [0.0, alert_height, 1.0, 1.3, -1.2, 16.0],                   # Rise to alert
            [-0.75, alert_height - 0.05, 0.95, 1.2, -1.9, 21.0],        # Snap look left
            [-0.7, alert_height, 1.0, 1.25, -0.85, 17.0],                 # Quick evaluation
            [-0.35, alert_height + 0.05, 1.05, 1.3, -1.7, 15.0],         # Scanning across
            [0.0, alert_height - 0.1, 0.95, 1.35, -1.0, 14.0],          # Pause - something?
            [0.35, alert_height, 1.02, 1.38, -1.8, 15.5],                # Continue scanning
            [0.8, alert_height - 0.05, 0.95, 1.42, -0.8, 20.5],         # Snap look right
            [0.75, alert_height - 0.15, 0.9, 1.38, -2.0, 18.0],         # Lock on target
            [0.72, alert_height - 0.2, 0.85, 1.35, -0.9, 16.5],         # Zoom in focus
            [0.7, alert_height - 0.18, 0.87, 1.37, -1.8, 14.5],         # Assess threat level
            [0.4, alert_height + 0.1, 1.1, 1.4, -1.1, 12.5],            # Relax - false alarm
            [-0.3, alert_height, 1.05, 1.35, -1.7, 15.0],               # One final check
            [0.0, -0.7, 1.15, 1.38, -1.3, 13.5],                        # All clear, lowering
            [0.0, -0.82, 1.2, 1.39, -1.6, 12.0],                        # Return to rest
            [0.0, -0.65, 1.2, 1.0, -1.5, 13.0]                          # Home 2
        ]
...
class SettlingAdjustAnimation(AnimationPlugin):
    ...
    def get_keyframes(self) -> Tuple[List[List[float]], List[float]]:
        ...
        keyframes = [
            ...
            [base_adj * 0.3, -0.78, 1.35, 1.35, -1.3, 15.0],                      # Uncomfortable
            [base_adj * 0.5 - 0.2, -0.95 + shoulder_var, 1.2, 1.5, -1.8, 14.5],   # Roll shoulder left
            [base_adj * 0.4, -0.82, 1.32, 1.38, -0.9, 16.0],                      # Test this spot
            [base_adj * 0.6, -0.85 + shoulder_var*0.5, 1.28, 1.42, -2.0, 15.5],   # Nope, adjust more
            [base_adj * 0.5 + 0.2, -0.95 - shoulder_var, 1.2, 1.5, -0.8, 14.0],   # Roll shoulder right
            [-base_adj * 0.3, -0.75, 1.35, 1.35, -1.7, 16.5],                     # Try leaning
            [-base_adj * 0.5, -0.88, 1.27, 1.43, -1.1, 15.0],                     # Still not comfortable
            [base_adj * 0.7, -0.6, 1.4, 1.2, -1.9, 17.0],                         # Big position change
            [base_adj * 0.3, -0.8, 1.3, 1.4, -0.85, 13.5],                         # This is better
            [base_adj * 0.15, -0.82, 1.31, 1.39, -1.6, 12.5],                     # Tiny adjustment
            [base_adj * 0.1, -0.83, 1.3, 1.4, -1.2, 11.5],                        # Another tiny one
            [0.05, -0.84, 1.3, 1.4, -1.7, 11.0],                                  # Almost perfect
            [0.0, -0.845, 1.3, 1.4, -1.3, 10.5],                                  # One final shift
            [0.0, -0.85, 1.1, 1.4, -1.6, 10.0],                                   # Ahh, comfortable
            [0.0, -0.65, 1.2, 1.0, -1.5, 12.0]                                    # Home 2
        ]
...
class DreamyDriftAnimation(AnimationPlugin):
    ...
    def get_keyframes(self) -> Tuple[List[List[float]], List[float]]:
        ...
        keyframes = [
            [0.0, -0.65, 1.2, 1.0, -1.5, 10.0],                           # Home 2
            [0.08, -0.88, 1.27, 1.43, -1.3, 10.5],                        # Subtle drift start
            [0.15, -1.05, 1.05, 1.65, -1.8, 10.0],                        # Float gently up
            [-drift_radius*0.8, -1.0, 1.1, 1.6, -0.85, 10.5],              # Arc to left high
            [-drift_radius, -0.95, 1.15, 1.55, -2.1, 10.0],                # Suspend at peak
            [drift_radius*0.6, -0.85, 1.25, 1.45, -0.8, 10.5],             # Fall gently right
            [drift_radius*0.9, -0.55, 1.45, 1.15, -1.9, 11.0],            # Swoop down low
            [drift_radius*0.7, -0.75, 1.3, 1.35, -1.0, 10.5],             # Begin rise
            [0.0, -0.9, 1.2, 1.5, -1.7, 10.0],                            # Cross center of 8
            [-drift_radius*0.7, -0.8, 1.25, 1.4, -0.9, 10.5],             # Float to opposite
            [-0.2, -0.65, 1.35, 1.25, -1.8, 11.5],                        # Slow downward spiral
            [0.0, -0.75, 1.28, 1.35, -1.1, 11.0],                         # Gravity slowly returns
            [0.0, -0.82, 1.29, 1.38, -1.6, 10.5],                         # Soft landing approach
            [0.0, -0.84, 1.2, 1.39, -1.4, 10.0],                          # Almost settled
            [0.0, -0.65, 1.2, 1.0, -1.5, 10.0]                            # Home 2
        ]
...
class NeckStretchAnimation(AnimationPlugin):
    ...
    def get_keyframes(self) -> Tuple[List[List[float]], List[float]]:
        ...
        keyframes = [
            [base_pos, -0.65, 1.2, 1.0, -1.5, 12.0],                    # Home 2
            [base_pos, -0.8, 1.25, 1.35, -1.3, 11.0],                   # Prepare for stretch
            [base_pos - 0.4, -0.75, 1.2, 1.3, -1.9, 10.5],              # Tilt head left
            [base_pos - 0.6, -0.7, 1.15, 1.25, -0.8, 10.0],             # Deep stretch left side
            [base_pos - 0.3, -0.9, 1.1, 1.5, -2.2, 10.5],               # Roll head back
            [base_pos + 0.4, -0.75, 1.2, 1.3, -0.75, 10.5],              # Tilt head right
            [base_pos + 0.6, -0.7, 1.15, 1.25, -1.8, 10.0],             # Deep stretch right
            [base_pos + 0.3, -0.5, 1.35, 1.05, -1.0, 11.0],             # Roll head forward
            [base_pos, -0.4, 1.45, 0.95, -2.0, 10.5],                   # Chin to chest stretch
            [base_pos, -1.05, 0.9, 1.7, -0.85, 10.0],                    # Look way up, stretch throat
            [base_pos + 0.05, -0.85, 1.2, 1.4, -1.7, 16.0],             # Quick center adjustment
            [base_pos - 0.05, -0.85, 1.25, 1.4, -1.2, 15.5],            # Relief shake
            [base_pos, -0.83, 1.28, 1.39, -1.6, 12.5],                  # Final position adjust
            [base_pos, -0.84, 1.2, 1.4, -1.4, 11.5],                    # Relaxed after stretch
            [base_pos, -0.65, 1.2, 1.0, -1.5, 12.0]                     # Home 2
        ]
...
class SleepAnimation(AnimationPlugin):
    ...
    def get_keyframes(self) -> Tuple[List[List[float]], List[float]]:
        ...
        keyframes = [
            ...
            [base_pos, -1.75, 1.95, 1.4, -2.7, 9.0],       # Happy center again - balanced
        ]
...
class YawningStretchAnimation(AnimationPlugin):
    ...
    def get_keyframes(self) -> Tuple[List[List[float]], List[float]]:
        ...
        keyframes = [
            [base_pos, -0.65, 1.2, 1.0, -1.5, 12.0],                    # Home 2
            [base_pos, -0.75, 1.35, 1.35, -1.3, 11.5],                  # Feeling tired
            [base_pos, -0.9, 1.15, 1.5, -1.8, 10.5],                    # Yawn beginning
            [base_pos - 0.1, -1.0, 1.0, 1.65, -0.8, 10.0],              # Mouth opening wide
            [base_pos, -1.05, 0.8, 1.8, -2.1, 10.0],                    # Arms stretching up
            [base_pos + 0.1, -1.1, 0.6, 1.95, -0.75, 10.0],             # Full body extension (at limit)
            [base_pos, -1.08, 0.62, 1.93, -1.9, 10.0],                  # Hold at peak
            [base_pos - 0.05, -1.05, 0.7, 1.85, -0.85, 14.0],           # Shudder during release
            [base_pos, -1.0, 0.95, 1.6, -1.7, 11.0],                    # Arms coming down
            [base_pos, -0.8, 1.2, 1.4, -1.1, 12.0],                     # Yawn closing
            [base_pos, -0.78, 1.25, 1.38, -1.8, 13.0],                  # Sleepy blink
            [base_pos + 0.15, -0.82, 1.27, 1.4, -1.0, 15.5],            # Shake head to wake
            [base_pos, -0.84, 1.2, 1.4, -1.6, 12.5],                    # Back to normal
            [base_pos, -0.65, 1.2, 1.0, -1.5, 12.0]                     # Home 2
        ]
...
class ShoulderShimmy(AnimationPlugin):
    ...
    def get_keyframes(self) -> Tuple[List[List[float]], List[float]]:
        ...
        keyframes = [
            [base_pos, -0.65, 1.2, 1.0, -1.5, 14.0],                    # Home 2
            [base_pos, -0.75, 1.2, 1.35, -1.3, 16.0],                   # Get ready
            [base_pos + 0.2, -0.85, 1.15, 1.45, -1.8, 18.0],            # Right shoulder up
            [base_pos - 0.2, -0.85, 1.15, 1.45, -0.9, 18.5],            # Left shoulder up
            [base_pos + 0.15, -0.8, 1.18, 1.42, -2.0, 19.0],            # Quick right
            [base_pos - 0.15, -0.8, 1.18, 1.42, -0.8, 19.5],            # Quick left
            [base_pos + 0.25, -0.78, 1.22, 1.38, -1.8, 20.0],           # Double shimmy
            [base_pos - 0.25, -0.78, 1.22, 1.38, -1.0, 20.5],           # Other side
            [base_pos + 0.35, -0.7, 1.25, 1.35, -2.1, 21.0],            # Big shimmy right
            [base_pos - 0.35, -0.7, 1.25, 1.35, -0.75, 21.5],           # Big shimmy left
            [base_pos, -0.75, 1.2, 1.4, -1.7, 17.0],                    # Back to center
            [base_pos + 0.1, -0.8, 1.25, 1.38, -1.2, 16.0],             # Final little shake
            [base_pos, -0.83, 1.28, 1.39, -1.6, 13.0],                  # Cool down
            [base_pos, -0.65, 1.2, 1.0, -1.5, 14.0]                     # Home 2
        ]
...
class LookAroundCasual(AnimationPlugin):
    ...
    def get_keyframes(self) -> Tuple[List[List[float]], List[float]]:
        ...
        keyframes = [
            [base_pos, -0.65, 1.2, 1.0, -1.5, 13.0],                    # Home 2
            [base_pos, -0.78, 1.25, 1.38, -1.3, 14.0],                  # Slight head lift
            [base_pos - 0.4, -0.75, 1.22, 1.35, -1.8, 15.0],            # Casual glance left
            [base_pos - 0.35, -0.72, 1.2, 1.32, -0.9, 14.5],            # Hmm, what's that?
            [base_pos, -0.77, 1.23, 1.37, -1.7, 13.5],                  # Look back to center
            [base_pos + 0.3, -0.75, 1.22, 1.4, -1.1, 15.5],             # Glance right
            [base_pos + 0.35, -0.85, 1.15, 1.5, -2.0, 14.0],            # Look up and right
            [base_pos + 0.5, -0.8, 1.2, 1.45, -0.8, 13.0],              # Check behind
            [base_pos, -0.78, 1.24, 1.38, -1.8, 13.5],                  # Back to center
            [base_pos - 0.25, -0.76, 1.23, 1.36, -1.0, 16.0],           # Quick left check
            [base_pos, -0.8, 1.26, 1.38, -1.6, 12.5],                   # All good
            [base_pos, -0.84, 1.29, 1.39, -1.4, 11.5],                  # Settling back
            [base_pos, -0.65, 1.2, 1.0, -1.5, 13.0]                     # Home 2
        ]
...
class ContentedSighAnimation(AnimationPlugin):
    ...
    def get_keyframes(self) -> Tuple[List[List[float]], List[float]]:
        ...
        keyframes = [
            [base_pos, -0.65, 1.2, 1.0, -1.5, 12.0],                    # Home 2
            [base_pos, -0.8, 1.25, 1.35, -1.3, 11.0],                   # Prepare for deep breath
...
class TailWagAnimation(AnimationPlugin):
    ...
    def get_keyframes(self) -> Tuple[List[List[float]], List[float]]:
        ...
        keyframes = [
            [base_pos, -0.65, 1.2, 1.0, -1.5, 15.0],                    # Home 2
            [base_pos, -0.7, 1.15, 1.05, -1.3, 17.0],                   # Wind up
            [base_pos + 0.4, -0.6, 1.1, 0.95, -2.0, 20.0],              # Fast wag right
            [base_pos - 0.4, -0.6, 1.1, 0.95, -1.0, 20.0],              # Fast wag left
            [base_pos + 0.5, -0.55, 1.05, 0.9, -2.2, 19.0],             # Big wag right
            [base_pos - 0.5, -0.55, 1.05, 0.9, -0.8, 19.0],             # Big wag left
            [base_pos + 0.3, -0.62, 1.12, 0.98, -1.8, 21.0],            # Quick double 1
            [base_pos - 0.3, -0.62, 1.12, 0.98, -1.2, 21.0],            # Quick double 2
            [base_pos + 0.1, -0.68, 1.18, 1.08, -1.9, 18.0],            # Excited wiggle
            [base_pos, -0.5, 1.0, 0.8, -1.5, 17.0],                     # Happy bounce
            [base_pos - 0.2, -0.7, 1.2, 1.1, -1.6, 15.0],               # Slow wag
            [base_pos, -0.75, 1.25, 1.15, -1.5, 14.0],                  # Settle
            [base_pos, -0.65, 1.2, 1.0, -1.5, 13.0]                     # Home 2
        ]
